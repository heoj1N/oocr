...
def get_config(args):
    ...
    if "microsoft/trocr" in args.model:
        ...
        config["generation_config"] = GenerationConfig(max_length=196, num_beams=5)
...
def get_processor_and_model(args, logger):
    ...
    if "microsoft/trocr" in args.model:
        ...
        text_encoder = CustomEncoder(
            pretrained_bert_model="dumitrescustefan/bert-base-romanian-cased-v1"
        )
        ...
        model = CustomVisionEncoderDecoderModel(
            vision_encoder, text_encoder, text_decoder, vocab_size
        )

        model.config.decoder_start_token_id = (
            tokenizer.cls_token_id or tokenizer.bos_token_id
        )
    ...
    if hasattr(model, "config"):
        ...
        token_values = {
            token: getattr(model.config, token) for token in required_tokens
        }

        # Verify all required tokens are set
        missing_tokens = [
            token for token, value in token_values.items() if value is None
        ]
        if missing_tokens:
...
def train_step(
    model: nn.Module, batch: Dict[str, Tensor], device: torch.device, model_type: str
) -> Tensor:
...
class CustomEncoder(nn.Module):
    def __init__(
        self,
        pretrained_bert_model: str = "dumitrescustefan/bert-base-romanian-cased-v1",
    ):
...
class CustomDecoder(nn.Module):
    ...
    def forward(
        ...
        tgt_embedded = self.embedding(tgt) + self.positional_encoding(
...
class CustomVisionEncoderDecoderModel(nn.Module):
    ...
    def forward(self, pixel_values, input_ids=None, attention_mask=None, labels=None):
        ...
        logits = self.text_decoder(
            tgt=input_ids if input_ids is not None else labels, memory=vision_features
        )
