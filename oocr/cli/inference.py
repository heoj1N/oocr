    ...
    with pdfplumber.open(pdf_stream) as pdf:
            ...
            for word in words:
                ...
                if rect_width > 0 and rect_height > 0:
                    ...
                    page_image.draw_rect(
                        (x0, y0, x1, y1), fill=None, stroke="red", stroke_width=1
                    )
                else:
                    print(
                        f"Skipped drawing invalid box with coordinates: x0={x0}, y0={y0}, x1={x1}, y1={y1}"
                    )
        ...
        for word in words:
            ...
            if (x1 > x0) and (y1 > y0):
                ...
                print(
                    f"Skipped cropping invalid box with coordinates: x0={x0}, y0={y0}, x1={x1}, y1={y1}"
                )
...
def parse_args():
    ...
    # Required arguments
    parser.add_argument(
        "--image_path", type=str, required=True, help="Path to input image or PDF file"
    )
    ...
    parser.add_argument(
        "--checkpoint_path", type=str, default=None, help="Path to .pt checkpoint file"
    )
...
if __name__ == "__main__":
    ...
    if os.path.isdir(args.model_path):
        ...
        and os.path.isfile(args.checkpoint_path)
        and args.checkpoint_path.endswith(".pt")
    ):
        ...
        for key in [
            ...
            if key in checkpoint_state_dict:
                if checkpoint_state_dict[key].size() != state_dict[key].size():
                    print(
                        f"Resizing {key} from {checkpoint_state_dict[key].size()} to {state_dict[key].size()}"
                    )
                    checkpoint_state_dict[key] = torch.nn.functional.pad(
                        ...
                        (
                            ...
                            - checkpoint_state_dict[key].size(0),
                        ),
                    )
        ...
        print(
            "Invalid path provided. It must be either a Hugging Face model directory or a .pt checkpoint file."
        )
        sys.exit(1)

    # Inference
    if args.image_path.lower().endswith(".pdf"):
        extracted_text, image_parts = extract_text_ckpt(
            model, processor, file_bytes, args.draw, mode="pdf"
        )
    elif args.image_path.lower().endswith((".jpg", ".png")):
        extracted_text, image_parts = extract_text_ckpt(
            model, processor, file_bytes, args.draw, mode="img"
        )
    else:
