...
class IAMDataset(Dataset):
    ...
    def __init__(
        ...
        self.split = split
        self.transform = (
            get_train_augmentations() if split == "train" and use_augmentation else None
        )
            ...
            try:
                ...
                with Image.open(image_path) as img:
                    ...
                    if arr.shape[0] > 1 and arr.shape[1] > 1:
                        ...
                        print(
                            f"[WARNING] Skipped tiny image '{image_path}' with shape {arr.shape}"
                        )
            except Exception as e:
                print(
                    f"[ERROR] Could not open or convert '{image_path}'. Skipping. ({e})"
                )
    ...
    def create_dataset(cls, datapath, processor, fraction, test_frac, args, logger):
        ...
        train_df, remainder_df = train_test_split(
            df, test_size=0.4, random_state=args.seed
        )
        eval_df, test_df = train_test_split(
            remainder_df, test_size=0.5, random_state=args.seed
        )
        ...
        if args.testdata:
            ...
            new_test_df["file_name"] = new_test_df["file_name"].apply(
            ...
            new_test_df = new_test_df.sample(
                frac=test_frac, random_state=args.seed
            ).reset_index(drop=True)
            datasets["test"] = IAMDataset(
                df=new_test_df, processor=processor, max_target_length=128
            )
...
class MNISTDataset(Dataset):
    ...
    def __init__(
        self, root_dir, processor, df=None, max_target_length=128, downloadable=True
    ):
        from torchvision.datasets import MNIST
    ...
    def create_dataset(cls, datapath, processor, fraction, test_frac, args, logger):
        ...
        datasets = {
            "train": MNISTDataset(
                datapath, processor, df=train_df, use_augmentation=args.use_augmentation
            ),
            "eval": MNISTDataset(
                datapath, processor, df=eval_df, use_augmentation=args.use_augmentation
            ),
            "test": MNISTDataset(
                datapath, processor, df=test_df, use_augmentation=args.use_augmentation
            ),
        }
...
class SROIEDataset(Dataset):
    ...
    def _crop_image(self, image, coords):
        ...
        x_max, y_max = (
            min(image.width, int(x_max + padding)),
            min(image.height, int(y_max + padding)),
        )
    ...
    def create_dataset(cls, datapath, processor, fraction, test_frac, args, logger):
        ...
        try:
            ...
            if not os.path.exists(datapath) or not os.listdir(datapath):
                ...
                try:
                    ...
                    kaggle.api.dataset_download_files(
                        "urbikn/sroie-datasetv2", path=datapath, unzip=True
                    )

                    nested_dir = os.path.join(
                        datapath, "SROIE2019"
                    )  # The nested directory created
                    if os.path.exists(nested_dir):
                        try:
                            ...
                            logger.info(
                                "You may need to manually move files from SROIE2019/SROIE2019/* to SROIE2019/*"
                            )

                    if not os.path.exists(
                        os.path.join(datapath, "train")
                    ) or not os.path.exists(os.path.join(datapath, "test")):
                    ...
                    logger.info(
                        "   - Ensure file permissions are secure: chmod 600 ~/.kaggle/kaggle.json"
                    )
                    raise
            ...
            train_df = train_df.sample(
                frac=fraction, random_state=args.seed
            ).reset_index(drop=True)

            # Split training data into train and eval
            train_df, eval_df = train_test_split(
                train_df, test_size=0.2, random_state=args.seed
            )
...
class SimpleDataset(Dataset):
    ...
    def __init__(
        ...
        if df is not None:
            ...
            df = pd.read_csv(
                self.labels_file, sep="\t", header=None, names=["file_name", "text"]
            )
    ...
    def create_dataset(cls, datapath, processor, fraction, test_frac, args, logger):
        ...
        df["file_path"] = df["file_name"].apply(
            lambda x: os.path.join(datapath, "images", x)
        )
        ...
        if args.testdata:
            ...
            datasets["test"] = SimpleDataset(
                root_dir=args.testdata, processor=processor, df=new_test_df
            )
...
def validate_files(df, logger):
    ...
    if not missing_files.empty:
        logger.warning(
            f"Missing {len(missing_files)} files. Example: {missing_files.iloc[0]['file_path']}"
        )
...
def init_dataset(args, processor, fraction, test_frac, logger):
    ...
    datasets = dataset_class.create_dataset(
        args.data, processor, fraction, test_frac, args, logger
    )
    logger.info(f"Number of training examples: {len(datasets['train'])}")
    logger.info(f"Number of validation examples: {len(datasets['eval'])}")
    logger.info(f"Number of test examples: {len(datasets['test'])}")
        ...
        [
            ...
            A.OneOf(
                [
                    ...
                    A.ElasticTransform(
                        alpha=1, sigma=10, border_mode=0, value=(255, 255, 255), p=0.3
                    ),
                    A.Perspective(
                        scale=(0.02, 0.05),
                        keep_size=True,
            ...
            A.OneOf(
                [
                    A.GaussNoise(
                        var_limit=(5.0, 30.0), mean=0, per_channel=True, p=0.5
                    ),
                    A.MultiplicativeNoise(
                        multiplier=(0.93, 1.07), per_channel=True, p=0.5
                    ),
            ...
            A.OneOf(
                [
                    A.CoarseDropout(
                        max_holes=8, max_height=2, max_width=2, min_holes=2, p=0.5
                    ),
                    A.Spatter(
                        mean=0.05,
                        std=0.02,
                        gauss_sigma=2,
                        cutout_threshold=0.3,
                        intensity=0.3,
                        p=0.4,
                    ),
            ...
            A.OneOf(
                [
                    A.HueSaturationValue(
                        hue_shift_limit=5, sat_shift_limit=10, val_shift_limit=10, p=0.5
                    ),
                    A.RGBShift(
            ...
            A.OneOf(
                [
                    ...
                    A.RandomFog(
                        fog_coef_lower=0.1, fog_coef_upper=0.2, alpha_coef=0.1, p=0.2
                    ),
                ],
                p=0.3,
            ),
            ...
            A.PadIfNeeded(
                min_height=64, min_width=64, border_mode=0, value=(255, 255, 255), p=1.0
            ),
        ]
    )
