...
import numpy as np
import torch
from oocr.data.datasets.dataset import get_dataset
...
def compute_metrics(pred_str, label_str, logger):
    ...
    try:
        cer = cer_metric.compute(
            predictions=pred_str_filtered, references=label_str_filtered
        )
...
def compute_all_metrics(predicted_text, reference_text):
    ...
    norm_ld = (
        ldist / max(len(predicted_text), ref_len)
        if max(len(predicted_text), ref_len) > 0
        else 0.0
    )
    ...
    lev_acc = (
        1 - (ldist / ref_len)
        if ref_len > 0
        else (1.0 if len(predicted_text) == 0 else 0.0)
    )
...
def plot_metric(
    ...
    if train_data:
        ...
        plt.plot(
            x_values_train, y_values_train, label=f"Train {metric_name}", color="blue"
        )
    ...
    if test_data:
        ...
        plt.plot(
            x_values_test, y_values_test, label=f"Test {metric_name}", color="green"
        )
    ...
    plt.ylim(
        min(
            min(y_values_train, default=0),
            min(y_values_val, default=0),
            min(y_values_test, default=0),
        )
        * 0.9,
        max(
            max(y_values_train, default=1),
            max(y_values_val, default=1),
            max(y_values_test, default=1),
        )
        * 1.1,
    )
    ...
    if show_epoch and iters_per_epoch is not None and iters_per_epoch > 0:
        ...
        tick_positions = [
            epoch_idx * iters_per_epoch for epoch_idx in range(num_epochs + 1)
        ]
...
def save_checkpoint(
    ...
    for metric_name, (mode, display_name) in metrics_to_check.items():
        ...
        is_better = (
            current_value < best_value if mode == "min" else current_value > best_value
        )

        if is_better:
            updated_best[metric_name] = current_value
            checkpoint_path = os.path.join(
                output_dir, f"{model_name}-best-{metric_name}.pt"
            )
...
def setup_directories(args):
    ...
    if args.data is None:
        ...
        if (
            dataset_class
            and hasattr(dataset_class, "downloadable")
            and dataset_class.downloadable
        ):
            ...
            raise ValueError(
                f"No data path provided and dataset '{args.dataset}' is not downloadable"
            )
    ...
    dir_components = {
        ...
        "test_name": os.path.basename(os.path.normpath(args.testdata))
        if args.testdata
        else "",
...
def create_logger(output_dir):
    ...
    file_handler = FileHandler(
        os.path.join(output_dir, "training.log"), encoding="utf-8", errors="replace"
    )
